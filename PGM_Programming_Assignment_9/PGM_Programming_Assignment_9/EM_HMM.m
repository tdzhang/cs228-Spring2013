...
for iter=1:maxIter
  ...
  for action=1:L
    ...
    Factors = repmat(struct ('var', [], 'card', [], 'val', []), 1, 2 * M );
    cF = 1;
    

    Factors(cF).var = 1;
    Factors(cF).card = [ K ];
    Factors(cF).val = log(P.c);
    cF = cF + 1;
  
    
    for i=2:M
      next = actionData(action).marg_ind(i);
      current = actionData(action).marg_ind(i-1);
      Factors(cF).var = [i-1 i];
      Factors(cF).card = [ K K ];
      Factors(cF).val = log(P.transMatrix(:)');
      cF = cF + 1;
    end
    ...
    for i = 1:M
      Factors(cF).var = i;
      Factors(cF).card = K;
      Factors(cF).val = logEmissionProb(actionData(action).marg_ind(i), :);
      cF = cF + 1;
    end
    
    [Marginals PCalibrated] = ComputeExactMarginalsHMM(Factors);
    ...
    for i=1:length(actionData(action).pair_ind)
      indexp = actionData(action).pair_ind(i);
      fromF = i;
      toF = i+1;
      for j=1:length(PCalibrated.cliqueList)
        if all(ismember([fromF toF], PCalibrated.cliqueList(j).var))
          PairProb(indexp, :) = exp(PCalibrated.cliqueList(j).val - logsumexp(PCalibrated.cliqueList(j).val));
          break;
        end
      end
    end
