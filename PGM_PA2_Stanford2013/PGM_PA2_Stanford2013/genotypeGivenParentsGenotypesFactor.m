...
number = numAlleles+0.5*numAlleles*(numAlleles-1);
genotypeFactor.card=number*ones(1,3);

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for i=1:numAlleles   %Case 4, because later parts can overwrite the values assigned
    for j=1:numAlleles
        for k=1:numAlleles
            for l=1:numAlleles
                genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, k), allelesToGenotypes(k, l), allelesToGenotypes(i, j)], [number, number, number]))=0.25;
                genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, l), allelesToGenotypes(k, l), allelesToGenotypes(i, j)], [number, number, number]))=0.25;
                genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(j, k), allelesToGenotypes(k, l), allelesToGenotypes(i, j)], [number, number, number]))=0.25;
                genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(j, l), allelesToGenotypes(k, l), allelesToGenotypes(i, j)], [number, number, number]))=0.25;
            end
        end
    end
end
...
for i=1:numAlleles   
    for j=1:numAlleles
        for k=1:numAlleles
            genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, j), allelesToGenotypes(j, k), allelesToGenotypes(i, i)], [number, number, number]))=0.5;
            genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, k), allelesToGenotypes(j, k), allelesToGenotypes(i, i)], [number, number, number]))=0.5;
            genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, j), allelesToGenotypes(i, i), allelesToGenotypes(j, k)], [number, number, number]))=0.5;
            genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, k), allelesToGenotypes(i, i), allelesToGenotypes(j, k)], [number, number, number]))=0.5;
        end
    end
end
...
for i=1:numAlleles   
    for j=1:numAlleles
        if (i~=j)
            genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, j), allelesToGenotypes(i, j), allelesToGenotypes(i, j)], [number, number, number]))=0.5;
        end
    end
end
...
for i=1:numAlleles
    for j=1:numAlleles
        genotypeFactor.val(AssignmentToIndex([allelesToGenotypes(i, j), allelesToGenotypes(j, j), allelesToGenotypes(i, i)], [number, number, number]))=1;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%