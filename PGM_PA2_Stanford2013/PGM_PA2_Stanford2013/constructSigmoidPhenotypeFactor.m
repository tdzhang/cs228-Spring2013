...
number=size(alleleWeights{1},2);
phenotypeFactor.card=[2 number number number number];
...
for i=1:2
   for j=1: number
       for k=1: number
            for l=1: number
                 for m=1: number
                    phenotypeFactor.val(AssignmentToIndex([i j k l m], phenotypeFactor.card))=computeSigmoid(alleleWeights{1}(j)+alleleWeights{2}(k)+alleleWeights{1}(l)+alleleWeights{2}(m));
                    if(i==2)
                        phenotypeFactor.val(AssignmentToIndex([i j k l m], phenotypeFactor.card))=1-phenotypeFactor.val(AssignmentToIndex([i j k l m], phenotypeFactor.card));
                    end
                 end
            end
       end
