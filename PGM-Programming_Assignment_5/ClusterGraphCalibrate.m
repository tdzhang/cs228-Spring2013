...
for m = 1:length(edgeFromIndx),
    ...
    for t = 1:length(MESSAGES(i, j).var)
        MESSAGES(i, j).card(t) = P.clusterList(i).card(find(MESSAGES(i, j).var(t) == P.clusterList(i).var));
    end
...
while (1),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    temp = P.clusterList(i);
    for k = 1:N
        if ((P.edges(k, i) ~= 0) && (k ~= j))
            temp = FactorProduct(temp, MESSAGES(k, i));
        end
    end
    MESSAGES(i, j) = FactorMarginalization(temp, setdiff(P.clusterList(i).var, intersect(P.clusterList(i).var, P.clusterList(j).var)));
