...
if partId == 1
    randi('seed',1);
    out = '';
    for ind = 1:length(INPUT.NGNCInds1)
      [i j] = NaiveGetNextClusters(INPUT.treeClust,INPUT.NGNCInds1(ind));
      out = [out int2str(i) ' ' int2str(j) ' '];
    end
    out;
...
elseif partId == 2
    ...
    randi('seed',2);
    out = '';
    for ind = 1:length(INPUT.NGNCInds2)
      [i j] = NaiveGetNextClusters(INPUT.treeClust,INPUT.NGNCInds2(ind));
      out = [out int2str(i) ' ' int2str(j) ' '];
    end
    out;
...
elseif partId == 3
    ...
    for i = 1:size(toyOut.edges,1)
      ...
      for j =1:size(toyOut.edges,2)
        out = [out ' ' num2str(toyOut.edges(i,j))];
      end
    end
    out;
...
elseif partId == 4
    ...
    %CreateClusterGraph
    toyOut = CreateClusterGraph(INPUT.toyFac2,[]);
    %out = SerializeFactorsFgTol(toyOut.clusterList,.001,0);
    out = SerializeFactorsFg(toyOut.clusterList);
    for i = 1:size(toyOut.edges,1)
      out = [out sprintf('\n')];
    ...
    %CreateClusterGraph
    treeOut = CreateClusterGraph(INPUT.treeFac,[]);
    %out = [out sprintf('\n') SerializeFactorsFgTol(treeOut.clusterList,.001,0)];
    out = [out sprintf('\n') SerializeFactorsFg(treeOut.clusterList)];
    for i = 1:size(treeOut.edges,1)
      out = [out sprintf('\n')];
      for j =1:size(treeOut.edges,2)
        out = [out ' ' num2str(treeOut.edges(i,j))];
      end
    end
    out;
...
elseif partId == 5
  ...
  out = '';
  for i = 1:2
    out = [out num2str(CheckConvergence(INPUT.messages{1},INPUT.messages{i})) ' '];
  end
  out;
...
elseif partId == 7
    %ClusterGraphCalibrate
    [outCG outM] = ClusterGraphCalibrate(INPUT.toyClust);
    out = '';
    %out = SerializeFactorsFgTol(outCG.clusterList,.001,0);
...
elseif partId == 8
    %ClusterGraphCalibrate
    [outCG outM] = ClusterGraphCalibrate(INPUT.toyClust2);
    out = '';
    %out = SerializeFactorsFgTol(outCG.clusterList,.001,0);
    out = SerializeFactorsFg(outCG.clusterList);
    for i = 1:size(outCG.edges,1)
...
elseif partId == 9
    %ComputeApproxMarginalBP
    outM = ComputeApproxMarginalsBP(INPUT.toyFac,[]);
    %out = SerializeFactorsFgTol(outM,.001,0);
    out = SerializeFactorsFg(outM);
    out;
    
elseif partId == 10
    %ComputeApproxMarginalBP    
    outM = ComputeApproxMarginalsBP(INPUT.toyFac2,[]);
    %out = SerializeFactorsFgTol(outM,.001,0);
    out = SerializeFactorsFg(outM);
    out;
